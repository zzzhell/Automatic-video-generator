...
class Soundtrack(object):
    ...
    * images: A list of File objects representing the images in the soundtrack.
    ...
    def download(self, song_hit_obj, path='', makeDirs=True, formatOrder=None, verbose=False):
        ...
        files.append(getAppropriateFile(song_hit_obj, formatOrder)) # /s.
...
def download(soundtrackId, song_hit_obj, path='', makeDirs=True, formatOrder=None, verbose=False):
    """Download the soundtrack with the ID `soundtrackId`.
    See Soundtrack.download for more information.
    """
    return Soundtrack(soundtrackId).download(song_hit_obj, path, makeDirs, formatOrder, verbose)
